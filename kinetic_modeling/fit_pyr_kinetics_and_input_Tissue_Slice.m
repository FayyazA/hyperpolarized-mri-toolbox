function [params_fit, Sfit, ufit, error_metrics] = fit_pyr_kinetics_and_input_Tissue_Slice(S, TR, flips, params_fixed, params_est, noise_level, plot_flag,filename)
...
params_all = {'kPL', ...
    'R1P', 'R1L', ...
    'Rinj', 'Tarrival', 'Tbolus','FP','FL','kLP'};
params_default_est = [0.01, ...
    1/48, 1/22, ...
    10^9, 40, 8,0.15,0.6,0];
params_default_lb = [0.0001, ...
    1/51, 1/38, ...
    10^4, 12, 35,1E-08,1E-08,1E-14];
params_default_ub = [0.08, ...
    1/47, 1/10, ...
    value 60 150,10,10,0.01];
...
%             subplot(2,1,2)
            plot(t, Mxy(1,:)./10^2, 'b*--', t, Mxy(2,:), 'g*--', t, squeeze(Sfit(i,1,:))./10^2,'b-', t, squeeze(Sfit(i,2,:)),'g-', t, ufit(i,:)./10^2, 'k-')
            ax = gca;
            ax.FontSize = 16;
            h = xlabel('time (s)');
            set(h, 'FontSize', 30);
